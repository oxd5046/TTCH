    ...
    getUserInfo(accessToken).then((userInfo) => {
      ...
      if (userInfo) {
        ...
        if (!parent) {
          ...
          getParentCaregiversByUsername(accessToken).then((caregiverParent) => {
            setParentCaregivers(
              caregiverParent ? caregiverParent : []
            );
          });
        } else {
          ...
          getCaregivers(accessToken).then((caregiverData) => {
            ...
            setCaregivers(caregiverData ? caregiverData : []);
          });
      ...
      getCaregiveeRequestsSent(accessToken).then((request) => {
        setCaregiveeRequests(request ? request : []);
      });

      //load caregivees 
      getCaregivees(accessToken).then((caregivee) => {
        setCaregivees(caregivee ? caregivee : []);
      });
    });
  }, [isFocused]);
